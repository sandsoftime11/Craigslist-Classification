...
df = df.drop(columns = ['City', 'Text', 'Price', 'depth', 'download_timeout', 'download_slot','download_latency'])


Title= df["Title"].apply(nltk.word_tokenize).tolist()

#pre-processing data
lemmatizer = nltk.stem.WordNetLemmatizer()
title_processed=[]
for row in Title:
  title_processed.append([lemmatizer.lemmatize(token.lower()) for token in row if token not in stopwords.words('english') if token.isalpha() if len(token)>1])
...
vectorizer1.fit(title_processed)
Title_V1 = vectorizer1.transform(title_processed)
